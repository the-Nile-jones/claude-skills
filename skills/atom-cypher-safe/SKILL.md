...
3. **CONFORM** — assemble props to schema: assign next `atom_id` in the family (e.g. `A-PKM-15`), `status:'Candidate'`, `load_bearing`/`did_specific` as `"Yes"`/`"No"`, **`confirmation_count: 1`**, `source` = the MA session / origin, `tags` comma-string with cross-refs. ⚠ Do NOT copy a sibling atom's prop set blindly to infer the schema — most live atoms predate `confirmation_count` and omitting it is how the migration stalls (this is exactly how `X-EPI-04` was created without it on 2026-07-09).
