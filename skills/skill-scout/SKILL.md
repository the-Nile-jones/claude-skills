...
Captures the scan-and-triage procedure run ad hoc 3× in one session (NotebookLM docs, XDA engineer prompts, skill-hubs) before being made deterministic — the skill-candidate rule (a repeatable procedure seen 3+ times → route it to skill-creator rather than filing it as another one-off task). Dedup discipline mirrors a duplicate-source registry and the source-vetting rule above. Hardened via a 5×5 RMA/DMA pass 2026-06-15 (fixes: cold-session fit-context loading, fetch-before-baseline ordering, large-source pre-filter with logged drops, pattern-extraction boundary, per-source-type modes, trusted≠unverified).
